	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
	    ...
	    //System.out.println("-_____________-");
	     while(it.hasNext()){
	    	 ...
	    	 try {
				LinkedLifeDataServiceResponse.Result result = service.findLinkedLifeDataEntitiesPaged(text, 0, 1);
				
				List<LinkedLifeDataServiceResponse.Entity> entities = result.getEntities();
				//System.out.println(entities.size());
				for(int i = 0; i < entities.size(); i++){
					LinkedLifeDataServiceResponse.Entity en = entities.get(i);
					LinkedLifeDataServiceResponse.Relation relation = en.getRelations().get(0);
					//System.out.println(relation.getSubj() + "  " + relation.getPred() + " " + relation.getObj());
