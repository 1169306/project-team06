...
public class TypeUtil {
	...
	public static List<Token> getOrderedTokens(JCas jcas) {
		return JCasUtil.select(jcas, Token.class).stream()
				.sorted(Comparator.comparing(Token::getBegin))
				.collect(toList());
	}
	...
	public static AbstractQuery getAbstractQueriesCombined(JCas jcas) {
		List<QueryConcept> conceptsCombined = getAbstractQueries(jcas)
				.stream()
				.flatMap(
						a -> FSCollectionFactory.create(a.getConcepts(),
								QueryConcept.class).stream()).collect(toList());
	...
	public static <T extends SearchResult> List<T> rankedSearchResultsByScore(
			Collection<T> results, int hitSize) {
		List<T> sorted = results.stream()
				.sorted(SEARCH_RESULT_SCORE_COMPARATOR).limit(hitSize)
				.collect(toList());
		IntStream.range(0, sorted.size()).forEach(
				rank -> sorted.get(rank).setRank(rank));
		return sorted;
	}
	...
	public static Collection<ConceptSearchResult> getRankedConceptSearchResults(
			JCas jcas) {
		return rankedSearchResultsByRank(JCasUtil.select(jcas,
				ConceptSearchResult.class));
	}

	public static Collection<TripleSearchResult> getRankedTripleSearchResults(
			JCas jcas) {
		return rankedSearchResultsByRank(JCasUtil.select(jcas,
				TripleSearchResult.class));
	}
	...
	public static Range<Integer> spanRangeInSection(Passage passage) {
		return Range.closedOpen(passage.getOffsetInBeginSection(),
